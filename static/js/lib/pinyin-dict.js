...
window.Pinyin.toPinyin = function(word){
    var word = new String(word)
    var pinyin = ""
    for(var i=0;i<word.length;i++){
	var tunes = this.CharMap[word[i]]
	if(!tunes){
	    var tune = ""
	}else{
	    var tune = tunes.split(",")[0]
	}
	tune = new String(tune)
	var _tune = ""
	for(var j=0;j<tune.length;j++){
	    if(this.TuneMap[tune[j]]){
		_tune += this.TuneMap[tune[j]]
	    }else{
		_tune += tune[j]
	    }
	}
